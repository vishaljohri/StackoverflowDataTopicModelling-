...
public class CopyFromSqlToFilesMaintainStructure {
	...
	public static void main(String[] args) throws ClassNotFoundException,
		...
		while (true) {
			 ...
			 * "select Id, PostTypeId, ParentId, Body, Title, Tags, LastActivityDate from posts"
			 * +
			...
			while (rs.next()) {
				System.out.println("Sequence No = " + count++ + " and id = "
						+ rs.getString("Id"));
				String creationDate[] = rs.getString("CreationDate").split(
						"\\s+");
				String fullDate[] = creationDate[0].split("-");
				...
				if (rs.getString("PostTypeId").equals("1")) {
					...
					try {
						FileWriter foutQuestion = new FileWriter(
								dirNameQuestion + "\\" + rs.getString("Id")
										+ "_" + monthWithYear + ".txt", true);
					...
					for (int i = 1; i < tags.length; i++) {
						try {
							FileWriter foutTag = new FileWriter(tagDir
									+ tags[i], true);
							BufferedWriter broutTag = new BufferedWriter(
									foutTag);
				...
				// answer posts
					...
					try {
						FileWriter foutAnswer = new FileWriter(dirNameAnswer
								+ "\\" + rs.getString("Id") + "_"
								+ rs.getString("ParentId") + "_"
								+ monthWithYear + ".txt", true);
						BufferedWriter broutAnswer = new BufferedWriter(
								foutAnswer);
